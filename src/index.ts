...
async function main(): Promise<void> {
  ...
  const rcEnv = readEnvFile([
    'RC_CLIENT_ID', 'RC_CLIENT_SECRET', 'RC_JWT', 'RC_SERVER',
    'RC_BOT_CLIENT_ID', 'RC_BOT_CLIENT_SECRET', 'RC_BOT_TOKEN',
  ]);
  ...
  if (rcEnv.RC_CLIENT_ID && rcEnv.RC_CLIENT_SECRET && rcEnv.RC_JWT) {
    ...
    try {
      ...
      logger.error({ err }, 'RingCentral (REST API) channel failed to connect — service continues without it');
      channels.splice(channels.indexOf(rcChannel), 1);
    }
  }

  // Bot Add-in (bot identity / token auth) → jid prefix 'rcb:'
  if (rcEnv.RC_BOT_CLIENT_ID && rcEnv.RC_BOT_CLIENT_SECRET && rcEnv.RC_BOT_TOKEN) {
    ...
    try {
      ...
      logger.error({ err }, 'RingCentral (Bot Add-in) channel failed to connect — service continues without it');
